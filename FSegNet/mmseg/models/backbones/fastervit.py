...
class TokenInitializer(nn.Module):
    ...
    def forward(self, x):
        x = self.to_global_feature(x)  # 通过一个3*3卷积 和 步长为1的平均池化
        B, C, H, W = x.shape
...
class FasterViTLayer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        if self.transformer_block:  # 从第三个阶段开始
...
class FasterViT(nn.Module):
    ...
    def __init__(self,
        ...
        self._load_state_dict("fastervit-3 model path")
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # B, N, 1/4, 1/4
        outs = []
